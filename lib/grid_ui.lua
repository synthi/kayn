-- lib/grid_ui.lua v0.508
-- CHANGELOG v0.508:
-- 1. FIX FATAL: Implementado Safe 2D Indexing estricto en key() y redraw() para evitar crasheos por hardware no estándar.
        ...
        elseif node and node.type ~= "dummy" then
            ...
            if G.shift_held then
                GridUI.disconnect_timer = clock.run(function()
                    ...
                    if node.type == "out" then for dst = 1, 66 do if G.patch[node.id] and G.patch[node.id][dst] and G.patch[node.id][dst].active then G.patch[node.id][dst].active = false; if Matrix.disconnect then Matrix.disconnect(node.id, dst, G) else Matrix.update_destination(dst, G) end end end
                    elseif node.type == "in" then for src = 1, 66 do if G.patch[src] and G.patch[src][node.id] and G.patch[src][node.id].active then G.patch[src][node.id].active = false; if Matrix.disconnect then Matrix.disconnect(src, node.id, G) else Matrix.update_destination(node.id, G) end end end end
            ...
            elseif #GridUI.held_nodes == 2 then
                ...
                if n1 and n2 and n1.type ~= n2.type and n1.type ~= "dummy" and n2.type ~= "dummy" then
                    ...
                    if G.patch[src.id] and G.patch[src.id][dst.id] and G.patch[src.id][dst.id].active then
                        ...
                        if G.patch[src.id] and G.patch[src.id][dst.id] then G.patch[src.id][dst.id].active = true; if Matrix.connect then Matrix.connect(src.id, dst.id, G) else Matrix.update_destination(dst.id, G) end end
                    end
...
function GridUI.redraw(G, g)
    ...
    if GridUI.refresh_counter > 60 then for x = 1, 16 do for y = 1, 8 do if GridUI.cache[x] then GridUI.cache[x][y] = -1 end end end; GridUI.refresh_counter = 0 end
        ...
        for y = 1, 8 do
            ...
            if y == 8 and x <= 8 then
                ...
                elseif x >= 3 and x <= 8 then
                    local snap_id = x - 2; if G.active_snap == snap_id then b = 11 elseif G.snapshots[snap_id] and G.snapshots[snap_id].has_data then b = 7 else b = 1 end
                end
            else
                local module_idx = G.module_by_col[x]
                local is_even_module = module_idx and (module_idx % 2 == 0) or false
                -- SAFE 2D INDEXING
                local node = G.grid_map[x] and G.grid_map[x][y]
                local is_menu = (y == 4)
                ...
                if (node and node.type ~= "dummy") or is_menu then
                    ...
                    for _, h in ipairs(GridUI.held_nodes) do
                        ...
                        elseif h.node and node then
                            if h.node.type == "out" and node.type == "in" and G.patch[h.node.id] and G.patch[h.node.id][node.id] and G.patch[h.node.id][node.id].active then b = 10
                            elseif h.node.type == "in" and node.type == "out" and G.patch[node.id] and G.patch[node.id][h.node.id] and G.patch[node.id][h.node.id].active then b = 10 end
                        end
                    end
                    if node and G.focus.state == "idle" and b ~= 15 and b ~= 10 then
                        local has_connection = false
                        if node.type == "out" then for dst = 1, 66 do if G.patch[node.id] and G.patch[node.id][dst] and G.patch[node.id][dst].active then has_connection = true; break end end
                        elseif node.type == "in" then for src = 1, 66 do if G.patch[src] and G.patch[src][node.id] and G.patch[src][node.id].active then has_connection = true; break end end end
                        if has_connection then b = math.max(b, 8) end 
                    end
                end
            end
            if GridUI.cache[x] and GridUI.cache[x][y] ~= b then g:led(x, y, b); GridUI.cache[x][y] = b end
        end
    end
    g:refresh()
end
